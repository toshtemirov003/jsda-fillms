...
function domlaration(array, node) {
  for (film of array) {
    ...
    newText.textContent = `Overview: ${film.overview}`;
    newDate.textContent = `Release_date: ${film.release_date}`;
...
elSelectSort.addEventListener("change", function () {
  ...
  // films.forEach((f) => {
  //   f.title.includes(selectValSort)
  // });
